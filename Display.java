...
public class Display {
	...
	public void drawGrid(int[][] f, int[][] Manbat, int[][] flag, double Time) {
		int piece;
			...
			for (int j = 0; j < numcols; j++) {
				...
				if (pieceImage != null) {
					...
					if (Manbat[i][j] == 0) {
						...
						p.fill(p.color(0));
						p.textSize(16);
						p.text(piece, x + j * dx + dx / 2, y + i * dy + dy / 2);
					} else {
						if (flag[i][j] == 1) {
							p.fill(0xffff0000);
							p.rect(x + j * dx, y + i * dy, dx, dy);
							p.fill(p.color(0));
							p.textSize(16);
							p.text("F", x + j * dx + dx / 2, y + i * dy + dy / 2);
						} else {
							p.fill(0xAAAAAAAA);
							p.rect(x + j * dx, y + i * dy, dx, dy);
						}
						...
						if (Time != -1) {
							int num = (int) (System.currentTimeMillis() - Time) / 1000;

							p.textSize(32);
							p.fill(0xFF000000);
							p.text("Time elapsed: " + num, 20, 60);
						}
					}
				}
			}
